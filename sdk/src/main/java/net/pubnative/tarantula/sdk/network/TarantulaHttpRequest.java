...
import net.pubnative.tarantula.sdk.exception.TarantulaException;
...
public class TarantulaHttpRequest {
    private static final String TAG = TarantulaHttpRequest.class.getSimpleName();
    ...
    public interface Listener {
         ...
         */
        void onPNHttpRequestFinish(TarantulaHttpRequest request, String result);
         ...
         */
        void onPNHttpRequestFail(TarantulaHttpRequest request, Exception exception);
    }
    ...
    public void start(Context context, final String urlString, Listener listener) {
        ...
        if (context == null) {
        ...
        } else if (Tarantula.getDeviceInfo().getConnectivity() != DeviceInfo.Connectivity.NONE) {
            ...
            invokeFail(TarantulaException.REQUEST_NO_INTERNET);
        }
    }
    ...
    protected void doRequest(String urlString) {
        ...
        try {
            ...
            if (TextUtils.isEmpty(postJson)) {
                ...
                connection.setRequestProperty("Content-MD5", TarantulaCrypto.md5(postJson));
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                invokeFail(TarantulaException.extraException(errorData));
            }
    ...
    protected String stringFromInputStream(InputStream inputStream) throws TarantulaException {
        String result = null;
        ...
        try {
            ...
            throw TarantulaException.extraException(errorData);
        }
        return result;
    }
    ...
    protected void invokeFinish(final String result) {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (mListener != null) {
                    mListener.onPNHttpRequestFinish(TarantulaHttpRequest.this, result);
                }
                mListener = null;
            }
        });
    ...
    protected void invokeFail(final Exception exception) {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (mListener != null) {
                    mListener.onPNHttpRequestFail(TarantulaHttpRequest.this, exception);
                }
                mListener = null;
            }
        });
