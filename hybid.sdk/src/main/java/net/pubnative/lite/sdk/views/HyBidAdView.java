...
public class HyBidAdView extends FrameLayout implements RequestManager.RequestListener, AdPresenter.Listener, AdPresenter.ImpressionListener, VideoListener, MRAIDViewListener, MraidCloseAdRepo.ICloseAdObserver {
    ...
    public void load(String zoneId, Position position, Listener listener) {
        mPosition = position;
    ...
    public void load(String appToken, String zoneId, Listener listener) {
        mAppToken = appToken;
        mZoneId = zoneId;
        mListener = listener;
        if (HyBid.isInitialized()) {
    ...
    public void loadExchangeAd(String zoneId, Position position, Listener listener) {
        mPosition = position;
        loadExchangeAd(zoneId, listener);
    }

    public void loadExchangeAd(String zoneId, Listener listener) {
        loadExchangeAd(null, zoneId, listener);
    }

    public void loadExchangeAd(String appToken, String zoneId, Listener listener) {
        loadExchangeAd(null, appToken, zoneId, listener);
    }

    public void loadCustomExchangeAd(String customUrl, Listener listener) {
        loadExchangeAd(customUrl, "", "", listener);
    }

    public void loadExchangeAd(String customUrl, String appToken, String zoneId, Listener listener) {
        loadExchangeAd(customUrl, appToken, zoneId, listener, null);
    }

    public void loadExchangeAd(String customUrl, String appToken, String zoneId, Listener listener, String adFormat) {
        mAppToken = appToken;
        mZoneId = zoneId;
        mListener = listener;
        if (HyBid.isInitialized()) {
...
//        invokeOnImpression();
    ...
    protected void startTracking() {
        if (mPresenter != null) {
            mPresenter.startTracking(percentageVisible -> {
                if (mRequestManager != null) {
                    mRequestManager.sendAdSessionDataToAtom(mAd, percentageVisible);
                }
            });
        }
    }
    ...
    protected void setupAdView(View view) {
        if (mPosition == null) {
            ...
            if (view.getParent() == null) {
                LayoutParams adLayoutParams = new LayoutParams(width, height);
