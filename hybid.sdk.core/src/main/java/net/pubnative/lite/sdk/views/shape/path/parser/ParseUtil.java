...
class ParseUtil {
    ...
    static final Float convertUnits(String name, XmlPullParser atts, float dpi, float width, float height) {
        ...
        if (value == null) {
            ...
            return Float.parseFloat(value.substring(0, value.length() - 2)) * dpi / 72;
        } else if (value.endsWith("pc")) {
            return Float.parseFloat(value.substring(0, value.length() - 2)) * dpi / 6;
        } else if (value.endsWith("cm")) {
            return Float.parseFloat(value.substring(0, value.length() - 2)) * dpi / 2.54f;
        } else if (value.endsWith("mm")) {
            return Float.parseFloat(value.substring(0, value.length() - 2)) * dpi / 254;
        } else if (value.endsWith("in")) {
            return Float.parseFloat(value.substring(0, value.length() - 2)) * dpi;
