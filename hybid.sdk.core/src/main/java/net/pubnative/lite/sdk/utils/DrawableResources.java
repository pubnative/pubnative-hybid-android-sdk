// HyBid SDK License
//
// https://github.com/pubnative/pubnative-hybid-android-sdk/blob/main/LICENSE
//
package net.pubnative.lite.sdk.utils;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.drawable.BitmapDrawable;
import android.graphics.drawable.Drawable;
import android.util.Base64;
import android.util.DisplayMetrics;

public class DrawableResources {

    String INTERSTITIAL_CLOSE_BUTTON_NORMAL_Encoded = "";
    String INTERSTITIAL_CLOSE_BUTTON_PRESSED_Encoded = "";

    public DrawableResources() {
    }

    public Drawable createDrawable(final Context context, Integer xmlResource, DrawableState state) {
        Bitmap bitmap = getBitmap(context, xmlResource, state);
        BitmapDrawable drawable = new BitmapDrawable(context.getResources(), bitmap);

        final DisplayMetrics displayMetrics = context.getResources().getDisplayMetrics();
        drawable.setTargetDensity(ViewUtils.asIntPixels(displayMetrics.xdpi, context));
        return drawable;
    }

    public Bitmap getBitmap(Context context, Integer xmlResource, DrawableState state) {
        if (xmlResource == null || xmlResource == -1) {
            String encodedString;
            if (state == DrawableState.PRESSED) {
                encodedString = INTERSTITIAL_CLOSE_BUTTON_PRESSED_Encoded;
            } else {
                encodedString = INTERSTITIAL_CLOSE_BUTTON_NORMAL_Encoded;
            }
            byte[] rawImageData = Base64.decode(encodedString, Base64.DEFAULT);
            return BitmapFactory.decodeByteArray(rawImageData, 0, rawImageData.length);
        } else {
            return BitmapFactory.decodeResource(context.getResources(), xmlResource, null);
        }
    }

    public enum DrawableState {
        NORMAL, PRESSED
    }
}
